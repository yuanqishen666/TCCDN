...
def eval_all(args, config, network):
    ...
    testDir = os.path.join('../model', args.model_dir, config.test_dir) #modify[1] 08-3 weida
    misc_utils.ensure_dir(testDir)
    ...
    # load data
    crowdhuman = CrowdHuman(config, if_train=False, if_test = True)
    ...
    for p in procs:
        p.join()
    fpath = os.path.join(testDir, 'dump-{}.json'.format(args.resume_weights))
    misc_utils.save_test_json_lines(all_results, fpath)
    # evaluation
    ...
    # eval_fid.write(line+'\n')
    # eval_fid.close()
    print("testing result saved in ", fpath)
...
def inference(config, network, model_file, device, dataset, start, end, result_queue):
    ...
    # inference
    for (image, im_info, ID) in data_iter:
        ...
        result_dict = dict(ID=ID[0], height=int(im_info[0, -2]), width=int(im_info[0, -1]),
