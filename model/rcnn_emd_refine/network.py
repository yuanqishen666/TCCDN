...
class Network(nn.Module):
    def __init__(self):
        ...
        self.RCNN = RCNN()
        assert config.num_classes == 3, 'Current Only support two class(1fg/1bg).'

    def forward(self, image, im_info, gt_boxes=None):
        # 秀的一批, 整到forward里面来标准化
        # image_mean.shape: [3] --> [1, 1, 3, 1] || image_std.shape: [3] --> [1, 1, 3, 1]
        image = (image - torch.tensor(config.image_mean[None, :, None, None]).type_as(image)) / (
                torch.tensor(config.image_std[None, :, None, None]).type_as(image))
        # before pad torch.Size([2, 3, 800, 1088])
        image = get_padded_tensor(image, 64)
        # after pad torch.Size([2, 3, 832, 1088])
        if self.training:
    ...
    def _forward_train(self, image, im_info, gt_boxes):
        # gt_boxes.shape = torch.Size([2, 500, 5])
        # backbone
        ...
        for feat in fpn_fms:
            print(feat.shape)
        '''
        # torch.Size([2, 256, 13, 22])
        # torch.Size([2, 256, 26, 44])
        # torch.Size([2, 256, 52, 88])
        # torch.Size([2, 256, 104, 176])
        # torch.Size([2, 256, 208, 352])
        ...
        rcnn_rois, rcnn_labels, rcnn_bbox_targets = fpn_roi_target(
                ...
                im_info,
                gt_boxes,
                top_k=2  # 这个top_k是不是一个anchor预测一个set{}, 里面有两个物体
        )
        # rcnn_rois.shape = torch.Size([1024, 5])
...
class RCNN(nn.Module):
    def __init__(self):
        ...
        # box predictor
        # config.num_classes = 2; bg:0, fg:1

        # 这里有两个head，是对应top_k=2吗？？？？？？
    ...
    def forward(self, fpn_fms, rcnn_rois, labels=None, bbox_targets=None):
        ...
        # pool_features.shape = torch.Size([1024, 256, 7, 7])

        flatten_feature = torch.flatten(pool_features, start_dim=1)
        # flatten_feature.shape = torch.Size([1024, 12544])

        flatten_feature = F.relu_(self.fc1(flatten_feature))  # 12544-->1024
        flatten_feature = F.relu_(self.fc2(flatten_feature))  # 1024 -->1024
        # flatten_feature.shape = torch.Size([1024, 1024])

        pred_emd_cls_0 = self.emd_pred_cls_0(flatten_feature)
        pred_emd_delta_0 = self.emd_pred_delta_0(flatten_feature)
        # pred_emd_cls_0 torch.Size([1024, 2]) pred_emd_delta_0 torch.Size([1024, 2 * 4])

        pred_emd_cls_1 = self.emd_pred_cls_1(flatten_feature)
        pred_emd_delta_1 = self.emd_pred_delta_1(flatten_feature)
        # pred_emd_cls_1 torch.Size([1024, 2]) pred_emd_delta_1 torch.Size([1024, 2 * 4])

        pred_emd_scores_0 = F.softmax(pred_emd_cls_0, dim=-1)
        pred_emd_scores_1 = F.softmax(pred_emd_cls_1, dim=-1)

        # cons refine feature  (微调网络)可选
